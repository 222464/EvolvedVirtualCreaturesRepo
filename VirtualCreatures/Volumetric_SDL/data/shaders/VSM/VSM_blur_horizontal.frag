...
void main()
{
	...
	vec3 moments = texture2D(scene, vec2(gl_TexCoord[0].x - halfNumSamples * blurSize, gl_TexCoord[0].y)).rgb;

	vec3 sum = moments.rgb * samples[0];

	float maxDepth = max(moments.x, moments.z);

	for(int i = 1; i < numSamples; i++)
	{
		moments = texture2D(scene, vec2(gl_TexCoord[0].x + (i - halfNumSamples) * blurSize, gl_TexCoord[0].y)).rgb;
